...
| Auth | Bot-first IdP → one-time Valkey token → Valkey-backed session; email/phone только recovery | A01 |
...
- Bot = IdP: `/start` → 10-minute Valkey token → safe GET/POST confirmation → Valkey
  session cookie.
- Единый аккаунт с explicit provider linking; автоматического account merge нет.
...
[deep-link URL]──▶[Web (htmx+alpine+MapLibre)]
	       ...
	       revisions/     encrypted   sessions/    notifications
	       geometry       raw/export  streams
```
