...
- Valid same-user current browser session превращает `/auth` rotation в credential
  ...
  создаётся. Без valid same-user session commit создаёт provisional ordinary login, а
...
- Rotated same-browser session сохраняет original `created_at`, получает новый token
  ...
  User-Agent; sliding TTL начинается на один год после provisional claim. Отдельного
...
- Session cookie содержит 128-bit opaque token; claimed session имеет one-year sliding
  TTL, provisional session — exact auth-flow deadline до first validated request.
...
- При valid current session того же user consume является same-browser credential
  refresh без `new_session` notification; иначе commit создаёт provisional ordinary
  login, а locked-on notification появляется только при claim. Other sessions re-auth
  не меняет.
- Same-browser record сохраняет original `created_at`, обновляет
  token/CSRF/freshness/activity metadata; one-year sliding TTL начинается после
